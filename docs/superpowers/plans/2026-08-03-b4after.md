...
- Modify: `src/db/shots.ts`, `src/ui/CameraScreen.tsx`, `src/ui/AlignScreen.tsx`, `src/App.tsx`, `e2e/flow.spec.ts`
...
Dans `src/ui/CameraScreen.tsx` et `src/ui/AlignScreen.tsx`, importer `isQuotaError` depuis `@/db/storage` et remplacer le `catch` de l'enregistrement par :

  ...
  await page.evaluate(
    () =>
      new Promise<void>((resolve) => {
        const request = indexedDB.deleteDatabase('b4after')
        request.onsuccess = () => resolve()
...
Exporter les deux fonctions utilitaires pour les tâches suivantes : ajouter `export` devant `resetDb` et `seed`.
...
Expected: PASS — les tests unitaires, les 12 tests de rendu et les 3 tests de parcours.

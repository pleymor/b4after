...
Le second test dépend de l'écran de calage créé par la Task 12 ; à ce stade il suffit que la route existe et affiche quelque chose. Le marquer `test.fixme` si la Task 12 n'est pas encore faite, puis retirer le marqueur.
...
Run: `npx playwright test --project=dev e2e/flow.spec.ts`
Expected: PASS pour le test du fantôme. Le test « mène à l écran de calage » reste en `fixme` jusqu'à la Task 12.
...
Ajouter à `e2e/flow.spec.ts` (et retirer le `test.fixme` du test de la Task 11) :
...
Run: `npx playwright test --project=dev e2e/flow.spec.ts`
Expected: PASS, y compris le test de la Task 11 dont le `fixme` a été retiré.

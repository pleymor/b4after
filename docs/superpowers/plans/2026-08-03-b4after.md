...
- Export JPEG : **2048 px maximum** sur le plus grand côté, qualité **0.85**. **Jamais d'agrandissement** — le facteur d'échelle est `Math.min(1, 2048 / maxEdge)`.
...
- Produces:
  - `openDb(): Promise<IDBPDatabase<B4Schema>>`, `resetDbForTests(): void` depuis `@/db/schema`
...
/** Oublie la connexion mémoïsée. Réservé aux tests, qui recréent la base entre chaque cas. */
...
**Files:**
- Create: `src/render/gif.worker.ts`, `src/render/gif.ts`
...
- [ ] **Step 4: Implémenter l'orchestration**

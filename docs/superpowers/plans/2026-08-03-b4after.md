...
- Consumes: rien
- Produces:
  - `ensurePersistence(): Promise<boolean>` — pour le chemin d'écriture : demande `navigator.storage.persist()` une seule fois par session, ne rejette jamais
  - `requestPersistence(): Promise<boolean>` — pour le bouton des réglages : demande explicite, **non mémoïsée**, ne rejette jamais
